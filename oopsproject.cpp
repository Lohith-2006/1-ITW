...
class User {
 protected:
    string username;
    string password;
    string role; // "admin", "student", "faculty"
...
public:
    ...
    void makePayment(float amount) {
        if (paidAmount + amount <= totalAmount) {
            ...
            displayDueAmount(); // Show due amount after payment
        } else {
    ...
    void viewFees() const {
        ...
        displayDueAmount(); // Show due amount when viewing fees
    }
...
private:
    ...
    map<string, int> attendance; // Mapping of course code to attendance percentage
    map<string, map<string, float>> examScores; // Course code -> exam type -> score
    Fee* feeInfo; // Pointer to Fee object
...
public:
    Student(string u, string p, int id, int age, string course, float feeAmount)
        ...
        feeInfo = new Fee(feeAmount); // Initialize fee information
    }

    ~Student() {
        delete feeInfo; // Clean up fee information
    }
    ...
    void addCourse(const Course &course) {
        ...
        attendance[course.getCourseCode()] = 0; // Initial attendance is 0%
        examScores[course.getCourseCode()] = {}; // Initialize empty map for exam scores
    }
...
private:
    ...
    map<int, map<string, int>> studentAttendance; // Nested map: student ID -> course code -> attendance %
    map<int, map<string, map<string, float>>> studentExamScores; // Nested map for exam scores
    map<int, map<string, float>> studentFinalGrades; // student ID -> course code -> final grade
